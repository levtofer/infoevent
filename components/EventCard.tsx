...
import { Calendar, MapPin, ChevronLeft, ChevronRight, ExternalLink, Pencil } from "lucide-react";
...
export default function EventCard({ event, isAdmin = false }: EventCardProps) {
  ...
  const images = event.galleryUrls && event.galleryUrls.length > 0
    ? event.galleryUrls
      ...
      <div className="relative sketch-border sketch-shadow bg-white/70 overflow-hidden hover:-translate-y-0.5 transition-transform flex flex-col group">
        {isAdmin && (
          <button
            ...
            title="Edit event"
            className="absolute top-2.5 right-2.5 z-5 bg-cream hover:bg-rose text-ink p-1.5 rounded-full border-2 border-ink sketch-shadow-sm transition"
          >
                  ...
                  {images.map((_, idx) => (
                    <span
                      ...
                      className={`w-1.5 h-1.5 rounded-full transition-all ${
                        idx === currentImageIndex ? "bg-rose w-3" : "bg-cream/60"
                      }`}
                    />
                  ))}
                </div>
